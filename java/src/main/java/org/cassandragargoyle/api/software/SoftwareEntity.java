...
public class SoftwareEntity extends AbstractEntity implements Software
{
	...
	public String getVersion() throws Exception
	{
		...
		if (exitCode == 0)
		{
			return readToString(process.getInputStream());
		}
		/* TODO:
		 * Error: 740
		 *
	...
	protected final boolean isInstalledByWhere(String name)
	{
		try
		{
			String command = OsDetector.isWindows() ? "where" : "which";
			Process process = new ProcessBuilder(command, name).start();
			int exitCode = process.waitFor();
			return exitCode == 0;
		}
	...
	public String commandWhere()
	{
		...
		{
			String command = OsDetector.isWindows() ? "where" : "which";
			var pb = new ProcessBuilder(command, getName());
			pb.redirectErrorStream(true);
	...
	public void install()
	{
		...
		CodeLanguage cl = null; //TODO:ZK How to select the language?
