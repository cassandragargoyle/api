...
public class Logging
{
	...
	private static final String ROOT_LOG = "org.cassandragargoyle.api";

	private static NonCloseHandler fileHandler;

	private static NonCloseHandler streamHandler;

	private static final Map<Throwable, Integer> CATCH_INDEX = Collections.synchronizedMap(new WeakHashMap<Throwable, Integer>()); // #190623
	...
	private static synchronized NonCloseHandler createStreamHandler()
	{
	...
	private static synchronized NonCloseHandler getFileHandler() throws Exception
	{
		if (fileHandler != null)
		{
			return fileHandler;
		...
		if (fileHandler == null)
		{
			fileHandler = createStreamHandler();
		}
		return fileHandler;
	}
	 ...
	 */
	private static void cleanUpOldFiles(File targetDir, int expirationPeriod, boolean autoClean) throws Exception
	{
		if (autoClean)
		{
			if (!targetDir.exists())
	...
	private static final class NonCloseHandler extends Handler implements Runnable
	{
		...
		NonCloseHandler(Handler h, int delay)
		{
			delegate = h;
		...
		public void publish(LogRecord record)
		{
			...
			if (t != null)
			{
				...
				for (int i = 1; i <= Math.min(tStack.length, hereStack.length); i++)
				{
					...
					{
						CATCH_INDEX.put(t, tStack.length - i);
						break;
					}
				}
			}
	...
	private static final class CustomFormatter extends Formatter
	{
		...
		public String format(LogRecord record)
		{
			...
			printRecord(sb, record, new HashSet<>());
			return sb.toString();
		}
		 ...
		 * @param beenThere
		 */
		private void printRecord(StringBuilder sb, LogRecord record, Set<Throwable> beenThere)
		{
		...
		private static void doPrintStackTrace(PrintWriter pw, Throwable t, Throwable higher)
		{
			...
			Integer caughtIndex = CATCH_INDEX.get(t);
			for (int i = 0; i < end; i++)
			{
		 ...
		 * @param record
		 */
		private void addLoggerName(StringBuilder sb, LogRecord record)
		{
	...
	private static final class DefaultLoggerFilter implements Filter
	{
		...
		public boolean isLoggable(LogRecord record)
		{
			...
			else if ("org.netbeans.TopSecurityManager".equals(record.getLoggerName()))
			{
				...
				{
					String thrownMsg = record.getThrown().getMessage();
					if (thrownMsg.startsWith("Dangerous reflection access to sun.misc.GC"
						+ " by class org.apache.cxf.common.logging.JDKBugHacks detected!")
						|| thrownMsg.startsWith("Dangerous reflection access to sun.misc.GC$Daemon"
						+ " by class java.lang.Thread$1 detected!"))
					{
						return false;
					}
				}
