...
public class DateUtil
{
	...
	private static final int[] RG_DAYS =
	{
	...
	public static Date fromDayCount(int dayCount)
	{
		...
		else
		{
			...
			else
			{
				...
				month = dayCount / 29;
				if ((dayCount > RG_DAYS[month] + leap) || ((month < 2) && (dayCount > RG_DAYS[month])))
				{
					month++;
				}

				day = dayCount - RG_DAYS[month - 1];
				if (month > 2)
				{
					day -= leap;
				}
	...
	public static int toDayCount(Date date)
	{
		...
		int dayCount = (RG_DAYS[month - 1] + day);
